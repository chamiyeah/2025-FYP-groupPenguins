...
# Added feature selection capabilities
...
def load_and_splitbypatient(data, features):
    ...
    # Added handling of outliers using IQR method
    ...
    X = X.drop(columns=to_drop)  # Remove highly correlated features
    ...
    train_ids, test_ids = train_test_split(unique_patients, test_size=0.2, random_state=42, stratify=data.groupby('patient_id')['cancer'].mean() > 0.5)
...
def model_training(data, features):
    ...
    # Using the optimized pipeline from cross_validation
    pipe = Pipeline([
...
def prediction_evaluation(data, features, pipe, result_dir, name1, name2):
    ...
    # Added probability calibration for better threshold selection
    y_prob = pipe.predict_proba(X_test)[:, 1]
    
    # Added optimal threshold finding using F1 score
    ...
    print(f'Cancer images: {len(results_df[results_df["label"]==1])}')
    print(f'Benign images: {len(results_df[results_df["label"]==0])}')
