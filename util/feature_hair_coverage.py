...
def hair_coverage(img_org): #img_org is a grayscale image
    ...
    #reduces image noise, skin texture, and small bumps before edge detection to distinguish hair easier later
    ...
    laplacian_norm = cv2.normalize(laplacian_abs, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8) #scaling edges to 0–255 for thresholding to have predictable range

    #thresholds edge magnitudes for hair mask
    threshold_value = 0.3 * 255 # 30% corresponds to moderate strong edge so that we have also ligther hair (tested value)
