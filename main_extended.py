...
#     #Features extraction
#     data = feature_extraction_extended(metadata, str(image_dir), str(mask_dir), good_pics, result_dir, filter=True)
...
#     prediction_evaluation(data, features, pipe, result_dir)
...
def main_baseline_with_data(feature_path, result_dir, model_path=None):
    ...
    # Training decision tree model using selected features
    pipe = model_training(data, features)
    ...
    # Predicting labels and evaluating on test set
    prediction_evaluation(data, features, pipe, result_dir)
...
if __name__ == "__main__":
    base_dir = Path(__file__).parent.resolve()
    result_dir = base_dir / "result"
    feature_path = result_dir / "feature_dataset.csv"  # Update manually if different
    model_path = result_dir / "trained_DT_extended.joblib"  # File for saving the trained model
